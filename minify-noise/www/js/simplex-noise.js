 ...
 * Optimized for 2D/3D noise generation
 */

const simplex = (function() {
  'use strict';

  // Simplex noise gradient directions
  const grad3 = [
  ...
  // Extend permutation table to avoid overflow
  const perm = [];
  const permMod12 = [];
  for (let i = 0; i < 512; i++) {
    perm[i] = p[i & 255];
  ...
  function noise2D(xin, yin) {
    ...
    if (t0 < 0) { n0 = 0.0; } else { t0 *= t0; n0 = t0 * t0 * dot(grad3[gi0], x0, y0, 0); }
    let t1 = 0.5 - x1 * x1 - y1 * y1;
    if (t1 < 0) { n1 = 0.0; } else { t1 *= t1; n1 = t1 * t1 * dot(grad3[gi1], x1, y1, 0); }
    let t2 = 0.5 - x2 * x2 - y2 * y2;
    if (t2 < 0) { n2 = 0.0; } else { t2 *= t2; n2 = t2 * t2 * dot(grad3[gi2], x2, y2, 0); }
